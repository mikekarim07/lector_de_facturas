...
def main():
    ...
    #     st.info(f'Total de archivos en la carpeta: {total_archivos}')
        ...
        st.write(st.session_state.folderpath)
...
#     df_parse_xml32 = pd.DataFrame()  # Inicializar df_parse_xml32 como un DataFrame vacío
#     df_parse_xml33 = pd.DataFrame()  # Inicializar df_parse_xml32 como un DataFrame vacío
#     df_parse_xml4 = pd.DataFrame()  # Inicializar df_parse_xml32 como un DataFrame vacío
#     df_paser_xmlcomp33 = pd.DataFrame()  # Inicializar df_parse_xml32 como un DataFrame vacío
...
#     # Obtener los valores únicos de la columna "Año" del DataFrame CFDIs
#         anos_unicos = CFDIs['Año'].unique()

#     # Widget multiselect para filtrar por año
#         filtro_anos = st.multiselect('Filtrar por Año', anos_unicos)

#     # Obtener los valores únicos de la columna "Mes" del DataFrame CFDIs
#         meses_unicos = CFDIs['Mes'].unique()

#     # Widget multiselect para filtrar por mes
#         filtro_meses = st.multiselect('Filtrar por Mes', meses_unicos)

#     # Aplicar filtros al DataFrame CFDIs
#         df_filtrado = CFDIs.copy()
#         if filtro_anos:
#             df_filtrado = df_filtrado[df_filtrado['Año'].isin(filtro_anos)]
#         if filtro_meses:
...
# Codigo modificado para correr la versión 4.0 y con la funcion para 3.3
# import os
# import pandas as pd
# import xml.etree.ElementTree as ET
# import streamlit as st
...
#     uuid = root.find('cfdi:Complemento/tfd:TimbreFiscalDigital', namespaces=namespaces).attrib.get('UUID', '')
#     fecha_emision = root.attrib.get('Fecha', '')
#     # fecha_pago = root.find('cfdi:Complemento/pago10:Pago', namespaces=namespaces).attrib.get('FechaPago', '')
#     # forma_pago = root.find('cfdi:Complemento/pago10:Pago', namespaces=namespaces).attrib.get('FormaDePagoP', '')
#     # moneda_pago = root.find('cfdi:Complemento/pago10:Pago', namespaces=namespaces).attrib.get('MonedaP', '')
#     # monto_pago = root.find('cfdi:Complemento/pago10:Pago', namespaces=namespaces).attrib.get('Monto', '')
...
# def main():
#     st.title('Procesamiento de archivos XML')
...
#         for xml_path in xml_files:
#             try:
#                 xml_data = parse_xml4(xml_path)
#                 data.append(xml_data)
#             except Exception as e:
#                 st.warning(f'Error al procesar el archivo {xml_path}: {str(e)}')
        

#         if data:
#             df = pd.DataFrame(data)
#             st.success(f'Se han procesado {len(df)} archivos XML.')
#             st.dataframe(df)
#         else:
#             st.warning('No se pudo procesar ningún archivo XML.')
...
# #     nombre_emisor = emisor.attrib['nombre']
